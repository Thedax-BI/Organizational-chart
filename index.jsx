  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6 md:p-8">
      ...
      <style>{`
        ...
        .org-children:before { content: ""; position: absolute; top: -24px; left: 10px; right: 10px; height: 2px; background: #e5e7eb; }
        .org-branch { position: relative; padding-top: 24px; }
        .org-branch:before { content: ""; position: absolute; top: 0; left: 50%; width: 2px; height: 24px; background: #e5e7eb; transform: translateX(-50%); }
      `}</style>
    </div>
  );
}
...
function NavButton({ active, onClick, icon, children }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${active ? "bg-blue-600 text-white shadow-lg" : "bg-white text-gray-700 hover:bg-gray-100"}`}>
      {icon} {children}
    </button>
  );
}

function DashCard({ label, value, className = "" }) {
  return (
...
function ResumoPill({ label, value, color = "" }) {
  return (
    <div>
